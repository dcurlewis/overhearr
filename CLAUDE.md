...
**Library-sync worker** (`server/services/librarySyncWorker.ts`) is a separate periodic job (`LIBRARY_SYNC_INTERVAL_MS`, default 1 h) that mirrors Lidarr's `/api/v1/album` into the `LidarrLibraryAlbum` table so search / discover / album / artist responses can flag rows the user already owns (`inLibrary: true`). It is **deliberately a separate worker from reconciliation** — different cadence, different failure mode, different table — and the same `NODE_ENV=test` no-op pattern applies. Sync semantics are full-snapshot replace: every pass upserts the current Lidarr library and deletes any local rows not in the snapshot. Admins can force a pass via `POST /api/settings/library-sync`. Per-request enrichment is a single batched lookup in `libraryLookupService.getLibraryStatusBatch()` — both the album (PK lookup) and artist (groupBy on `foreignArtistId`) axes are answered without round-tripping to Lidarr.
...
**Image proxy + on-disk cache** (`server/services/imageCacheService.ts`, route `server/routes/image.ts`). Cover-art / artist images are never referenced upstream directly from the browser — the frontend rewrites every URL through `GET /api/image?src=<encoded-upstream-url>` via `proxiedImage()` (`src/lib/image.ts`). The endpoint fetches the upstream once, caches the bytes on disk under `IMAGE_CACHE_DIR` (default `/config/cache/images` in prod, `.cache/images` in dev) keyed by `sha1(url)` with a `.json` sidecar, ~7d TTL, and size-capped LRU eviction at `IMAGE_CACHE_MAX_BYTES` (default 512 MiB). It is **auth-gated** (`requireAuth`) — which is why the frontend uses `next/image` with `unoptimized`: Next's optimizer fetches server-side without the session cookie and couldn't reach the proxy. **SSRF is the core risk**: `assertAllowedImageUrl` allowlists scheme (http/https only) and host (CAA, MusicBrainz, Last.fm CDN, plus `archive.org`/`*.us.archive.org` for CAA's redirect targets), redirects are followed manually with the allowlist re-checked on *every hop*, and non-image content types are rejected. Adding an image source means editing the allowlist in `imageCacheService.ts`.
